...
- Amount input → `setFromRobux`/`setFromUsd` (derive the other currency via `DEVEX_RATE`) → `updateUI` full re-render. Entered amounts are in-memory only and reset on every page load.
...
- The footer status line (`#resetRateBtn`) is hidden while a live-rate request is in flight; the disabled, spinning refresh button conveys that state. Afterward, the line is shown for manual, cached, stale, and offline-default rates; clicking it clears a manual override or re-fetches otherwise. On a fresh live rate the line is hidden and the update time moves into the refresh button's label/title.

## Persistence

`STORAGE_KEYS` is the single registry of localStorage keys (theme, market rate + timestamp, manual rate, panel open). Amounts and entry mode are intentionally not persisted; startup clears any legacy amount keys. All access goes through try/catch wrappers `getStoredItem`/`setStoredItem`/`removeStoredItem` for embedded contexts without storage.

## Startup sequence

Bottom of `app.js`: initialize theme → restore cached/manual rates → `syncEffectiveRate` → clear legacy amount keys → apply default entry mode and panel visibility → `refreshTaxDisplays` → `updateUI` → initial fetch → register service worker (secure contexts only).
